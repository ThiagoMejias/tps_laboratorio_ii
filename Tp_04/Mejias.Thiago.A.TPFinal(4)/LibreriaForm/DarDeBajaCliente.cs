...
namespace LibreriaForm
{
    public partial class DarDeBajaCliente : Form
    {
        ...
        //TODO: revisar posibles excepciones salidas de base de datos (clietnedao);
            ...
            if (!(string.IsNullOrEmpty(rtb_Cliente.Text)))
            {
             ...
             "Confirmacion de baja", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                try
                {
                    ...
                    {
                        Cliente clienteABorrar = bacos.buscarCliente(int.Parse(this.txt_buscarPorDni.Text));
                        if (this.bacos.clientes.Remove(clienteABorrar))
                        {
                            ClienteDao.Borrar(clienteABorrar);
                ...
                {
                    MessageBox.Show(ex.Message, "Error eliminando de la base de datos", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
